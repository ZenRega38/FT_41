...
function GraduateWallContent() {
  ...
  const filteredParticipants = useMemo(() => {
    // Reset visible count whenever filter, search or sort changes
    return participants
  ...
  // Reset visibleCount whenever filter/search/sort changes
  useEffect(() => {
  ...
  const handleMobileScroll = (e: React.UIEvent<HTMLDivElement>) => {
    ...
    // Calculate index based on container width. We use container.clientWidth because each chunk is roughly 100% of container.
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
            <div className="relative w-full sm:w-auto">
              <select
                ...
                className="w-full sm:w-56 appearance-none bg-charcoal border border-glass focus:border-gold/50 rounded-lg py-2.5 pl-4 pr-10 text-sm text-text-primary outline-none transition-colors cursor-pointer"
          ...
          <>
            ...
            <div className="flex flex-col items-center gap-4 mt-8 md:mt-14">
              ...
              <p className="text-xs text-text-muted tracking-widest uppercase">
                Menampilkan halaman {mobileChunks.length > 0 ? activeScrollIndex + 1 : 0}/{mobileChunks.length}, {Math.min(visibleCount, filteredParticipants.length)} dari {filteredParticipants.length} lulusan
              </p>
